...
public class Graph implements IGraph {
	...
	public Graph(List<INode> states, List<IEdge> transitions) {
		mNodes = states;
		mEdges = transitions;
		for(IEdge e:transitions){
	...
	public IEdge addEdge(IEdge edge) {
		System.out.println("Adding EDGE MAN!!" + edge.getEdgeId() + " to " + Arrays.toString(mEdges.toArray()));
	...
	public INode getNodeById(int id)
	{
		...
		for(i=0;i<mNodes.size();i++)
			if(this.mNodes.get(i).getNodeId()==id)
